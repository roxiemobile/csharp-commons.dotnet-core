...
//    // allowing us to ask for a WinRT-specific XmlResourceManager.
...
//    }
    ...
    // Refactoring Notes
    // -----------------
    // In Feb 08, began first step of refactoring XmlResourceManager to improve
    ...
    // XmlResourceManager creates one of these classes based on whether the 
    ...
    // XmlResourceManager member UseManifest and UseSatelliteAssem are no 
    // longer relevant on XmlResourceManager. Similarly, other members could
    ...
    // members on XmlResourceManager but allow the file-based and manifest-
    // based classes to access/set these members in a uniform way. See
    // XmlBasedResourceManagerMediator.
    ...
    // - Makes it possible to understand what the XmlResourceManager does, 
    // which is key for maintainability. 
    // - Makes the XmlResourceManager more extensible by identifying and
...
//        private Assembly _callingAssembly;  // Assembly who created the ResMgr.
        ...
        private IXmlResourceGroveler _resourceGroveler;
...
//        //initializes assembly, which initializes XmlResourceManager, which tries to get a CultureInfo which isn't
...
//        // InternalResGen.
//        internal static readonly string ResReaderTypeName = typeof(XmlResourceReader).FullName;
//        internal static readonly string ResSetTypeName = typeof(RuntimeResourceSet).FullName;
//        internal static readonly string MscorlibName = typeof(XmlResourceReader).Assembly.FullName;
//        internal const string ResFileExtension = ".resources";
        internal const string ResFileExtension = ".xml";
...
//        }
//
//        protected XmlResourceManager()
//        {
//            Init();
//
//            _lastUsedResourceCache = new CultureNameResourceSetPair();
//            XmlBasedResourceManagerMediator mediator = new XmlBasedResourceManagerMediator(this);
...
//        // 
//        private XmlResourceManager(string baseName, string resourceDir, Type usingResourceSet)
//        {
//            if (null == baseName)
...
//
//        [System.Security.DynamicSecurityMethod] // Methods containing StackCrawlMark local var has to be marked DynamicSecurityMethod
        public XmlResourceManager(string baseName, Assembly assembly)
        {
            if (null == baseName)
                throw new ArgumentNullException(nameof(baseName));
            ...
            if (!(assembly is Assembly))
                throw new ArgumentException(Messages.Argument_MustBeRuntimeAssembly);

            MainAssembly = assembly;
            BaseNameField = baseName;

            // NOTE: Added by Stalker!
            moduleDir = Path.GetDirectoryName(assembly.Location);
...
//        [System.Security.DynamicSecurityMethod] // Methods containing StackCrawlMark local var has to be marked DynamicSecurityMethod
//        public XmlResourceManager(string baseName, Assembly assembly, Type usingResourceSet)
//        {
//            if (null == baseName)
//                throw new ArgumentNullException(nameof(baseName));
//            if (null == assembly)
...
//
//        [System.Security.DynamicSecurityMethod] // Methods containing StackCrawlMark local var has to be marked DynamicSecurityMethod
//        public XmlResourceManager(Type resourceSource)
//        {
...
//            UseManifest = true;
            ...
            XmlBasedResourceManagerMediator mediator = new XmlBasedResourceManagerMediator(this);
//          resourceGroveler = new ManifestBasedResourceGroveler(mediator);
            _resourceGroveler = new FileBasedXmlResourceGroveler(mediator);

//          _neutralResourcesCulture = ManifestBasedResourceGroveler.GetNeutralResourcesLanguage(MainAssembly, ref _fallbackLoc);
            _neutralResourcesCulture = CultureInfo.InvariantCulture;
        }

        // Gets the base name for the XmlResourceManager.
...
//        // Returns the Type of the ResourceSet the XmlResourceManager uses
...
//        }
//
//        // Tells the XmlResourceManager to call Close on all ResourceSets and 
//        // release all resources.  This will shrink your working set by
//        // potentially a substantial amount in a running application.  Any
//        // future resource lookups on this XmlResourceManager will be as 
...
//        // creating a new XmlResourceManager isn't quite the correct behavior.
...
//        }
//
//        public static XmlResourceManager CreateFileBasedResourceManager(string baseName, string resourceDir, Type usingResourceSet)
//        {
//            return new XmlResourceManager(baseName, resourceDir, usingResourceSet);
//        }
        ...
        // passing to the XmlResourceReader constructor) or a manifest resource file
        ...
        internal virtual string GetResourceFileName(CultureInfo culture)
        {
...
//          }
            ...
            sb.Append(BaseNameField.Replace('.', Path.DirectorySeparatorChar));
        ...
        // not useful for most users of the XmlResourceManager - call 
...
//            }
        ...
        private ResourceSet InternalGetResourceSet(CultureInfo requestedCulture, bool createIfNotExists, bool tryParents)
        {
            Dictionary<string, ResourceSet> localResourceSets = _resourceSets;
            ResourceSet rs = null;
            ...
            foreach (CultureInfo currentCultureInfo in mgr)
            {
                ...
                // XmlResourceManager).  It's happened.

                rs = _resourceGroveler.GrovelForResourceSet(currentCultureInfo, tryParents);
        ...
        private static void AddResourceSet(Dictionary<string, ResourceSet> localResourceSets, string cultureName, ref ResourceSet rs)
        {
            ...
            // back into the XmlResourceManager in unexpectedly on the same thread.
            lock (localResourceSets)
            {
...
//            // exception types that the XmlResourceManager class is not documented to throw.
...
//        }
        ...
        // Only call SetAppXConfiguration from XmlResourceManager constructors, and nowhere else.
        // Throws MissingXmlResourceException and WinRT HResults
...
//                        s_IsAppXModel = true;
//
//                        // If we have the type information from the XmlResourceManager(Type) constructor, we use it. Otherwise, we use BaseNameField.
//                        string reswFilename = _locationInfo == null ? BaseNameField : _locationInfo.FullName;
//
//                        // The only way this can happen is if a class inherited from XmlResourceManager and
//                        // did not set the BaseNameField before calling the protected XmlResourceManager() constructor.
...
//                        // XmlResourceManager() takes no arguments, and because it is not documented as throwing
...
//                        // throw a MissingXmlResourceException when GetString is called indicating that a
...
//                                // resources. We want to fail by throwing a MissingXmlResourceException
...
//                                // the MissingXmlResourceException from this function, but from GetString. See the
...
//                                    }
//                                    // We would like to be able to throw a MissingXmlResourceException here if PRI resources
//                                    // could not be loaded for a recognized reason. However, the XmlResourceManager constructors
//                                    // that call SetAppXConfiguration are not documented as throwing MissingXmlResourceException,
...
//                                    catch (Exception e)
//                                    {
//                                        // ERROR_MRM_MAP_NOT_FOUND can be thrown by the call to XmlResourceManager.get_AllResourceMaps
...
//                                    // of exceptions that the XmlResourceManager class is already documented as throwing.
//                                    // This would allow existing portable libraries to gracefully recover if they don't care
//                                    // too much about the XmlResourceManager object they are using. However it could
...
//                        throw new MissingXmlResourceException(SR.Format(SR.MissingManifestResource_ResWFileNotLoaded, _PRIExceptionInfo._ResWFile, _PRIExceptionInfo._PackageSimpleName));
//
//                    throw new MissingXmlResourceException(SR.MissingManifestResource_NoPRIresources);
//                }
//
...
//        }
        ...
        internal class XmlBasedResourceManagerMediator
        {
            private XmlResourceManager _rm;

            internal XmlBasedResourceManagerMediator(XmlResourceManager rm)
            {
                if (rm == null)
                {
...
//            {
//                return XmlResourceManager.GetSatelliteContractVersion(a);
//            }
//
